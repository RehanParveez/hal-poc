  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="bg-gray-800 text-white px-5 py-4">
      <div class="flex items-center justify-between">
        ...
        <span :class="['text-xs px-3 py-1 rounded-full font-semibold', invoice.grade_received === 'Grade A' ? 'bg-green-400 text-white' : 'bg-yellow-500 text-white']">
    ...
    <div v-if="parseFloat(invoice.farmer_net_profit) === 0 && invoice.insurance_claim_triggered" class="bg-red-50 px-5 py-3 text-sm text-red-700">
...
const rows = computed(() => {
  const list = [
    ...
    { label: 'FasalPay Platform Fee', value: props.invoice.platform_transaction_fee, type: 'deduction' },
  ]
