<template>
  <div>
    ...
    <template v-else>
      <p class="text-sm text-gray-500 mb-4">{{ contracts.openContracts.length }} {{ $t('farmer.openContracts').toLowerCase() }}</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="c in contracts.openContracts" :key="c.id" class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden card-hover">
          ...
          <div class="p-5">
            ...
            <p class="text-xs text-gray-500 mt-2">{{ c.required_kg - c.allocated_kg }} kg {{ $t('farmer.remaining').toLowerCase() }} · {{ $t('farmer.deadline').toLowerCase() }} {{ c.delivery_deadline }}</p>
            
            <div class="mt-4 flex gap-2">
              <input v-model.number="kgForm[c.id]" type="number" :placeholder="$t('farmer.committedQuantity')" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
              <button @click="handleAllocate(c.id)" class="btn-primary whitespace-nowrap">{{ $t('farmer.allocate') }}</button>
            </div>
          </div>
        </div>
      </div>
      
      <p v-if="contracts.openContracts.length === 0" class="text-gray-500 text-center py-8">{{ $t('farmer.noOpenContracts') }}</p>
    </template>
  </div>
</template>
...
function cropName(cropId) {
  ...
  return crop ? `${crop.name} (${crop.code})` : t('farmer.unknownCrop')
}

async function handleAllocate(contractId) {
  const kg = kgForm[contractId]
  if(!kg || kg <= 0) {
    notify.showError({ message: t('farmer.errorEnterKg') })
    return
  }
  if (!loans.activeLoan) {
    notify.showError({ message: t('farmer.errorNeedLoan') })
    return
  }
  try {
    ...
    if (error.response && error.response.data && error.response.data.error) {
      ...
      notify.showError({ message: t('farmer.errorAllocateFailed') })
    }
  }
}
</script>