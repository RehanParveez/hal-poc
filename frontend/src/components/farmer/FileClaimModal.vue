<template>
  <div class="fixed inset-0 bg-slate-900/70 flex items-center justify-center p-4 z-50">
    <div class="bg-slate-950 border border-slate-800 rounded-xl p-6 w-full max-w-md shadow-2xl">
      <h3 class="text-lg font-bold text-slate-200 mb-4">{{ $t('farmer.fileClaimBtn') }}</h3>
      
      <label class="block text-xs font-semibold text-slate-400 mb-1">{{ $t('common.select') }}</label>
      ...
      <label class="block text-xs font-semibold text-slate-400 mb-1">{{ $t('common.reason') }}</label>
      ...
      <label class="block text-xs font-semibold text-slate-400 mb-1">{{ $t('insurance.claimAmount') }} (PKR)</label>
      ...
      <div class="flex gap-2">
        <button @click="handleSubmit" :disabled="insurance.isLoading || !form.policy_id || !form.reason" 
          ...
          {{ insurance.isLoading ? $t('common.loading') : $t('common.submit') }}
        </button>
