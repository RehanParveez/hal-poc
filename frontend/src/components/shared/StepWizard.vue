      ...
      <template v-for="(step, i) in steps" :key="step.id">
        <div class="flex flex-col items-center">
          <div :class="['w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold',
            completedSteps.includes(i) ? 'bg-green-600 text-white' :
            i === currentStep ? 'bg-green-700 text-white' : 'bg-gray-200 text-gray-400']">
            <span v-if="completedSteps.includes(i)">✓</span>
            <span v-else>{{ i + 1 }}</span>
          </div>
          <p class="text-xs mt-1 text-gray-600">{{ step.label }}</p>
        </div>
        <div v-if="i < steps.length - 1" class="w-10 h-0.5 mx-1"
          :class="completedSteps.includes(i) ? 'bg-green-600' : 'bg-gray-200'" />
      </template>
    </div>
