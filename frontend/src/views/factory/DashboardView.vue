...
async function submitGrade(batchId) {
  ...
  if (!form.grade_received) {
    notify.showError('Select a grade before confirming.')
    return
  }
  if (form.grade_deduction_pct < 0 || form.grade_deduction_pct > 100) {
    notify.showError('Deduction percentage must be between 0 and 100.')
    return
  }
  try {
    await delivery.confirmGrade(batchId, form.grade_received, form.grade_deduction_pct, '')
  } catch (error) {
    notify.showError(error.response?.data?.error ?? 'Failed to confirm grade.')
  }
}
...
async function handleFactorySettle(invoiceId) {
  try {
    ...
    notify.showError(error.response?.data?.error ?? 'Failed to settle with bank.')
  }
}
